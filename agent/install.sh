...
sudo() {
    ...
    if [ "$myEUID" -ne 0 ]; then
        if command -v sudo >/dev/null 2>&1; then
            ...
            err "ERROR: sudo not installed, cannot proceed."
            exit 1
        fi
    else
        "$@"
...
# 安装 OpenRC 和 rc-update
install_openrc() {
    if ! command -v rc-update >/dev/null 2>&1; then
        info "Installing OpenRC and rc-update..."
        sudo apk update
        sudo apk add openrc
    fi
}
...
install() {
    ...
    if ! sudo rc-update add nezha-agent default; then
        err "Failed to add nezha-agent to rc-update."
        exit 1
    fi
